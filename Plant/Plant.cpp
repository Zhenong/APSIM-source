...
//- Implementation Section ----------------------------------
   ...
   co2Modifier->read_co2_constants ();
    ...
    //  stress switch (1 for stress, 0 for no stress)
    c.switch_temp = 1;
    c.switch_drought = 1;
    if(c.crop_type=="soybean"){
    	scienceAPI.read("switch_temp", c.switch_temp, 0, 1);
    	scienceAPI.read("switch_drought", c.switch_drought, 0, 1); 
    	}
   }
...
float Plant::getTempStressPhoto(void)  {			// added by ZN-Jin
	if(c.switch_temp==1){ return tempStress->tFact.photo;}
	else{ return 1.0;}	
}
...
float Plant::getSwdefPhoto(void){				// added by ZN-Jin
	if(c.switch_drought==1){ return swStress->swDef.photo;}
	else{ return 1.0;}	
} 
